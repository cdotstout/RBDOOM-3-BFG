...
void idRenderProgManager::Init() {
	...
	m_renderProgs.SetNum( numBuiltins );
	...
	for ( int i = 0; i < numBuiltins; i++ ) {
		...
		if ( builtins[ i ].stages & SHADER_STAGE_VERTEX ) {
			...
			if (vIndex < 0) {
				idLib::Printf("Couldn't find shader %s\n", builtins[i].name);
				continue;
			}
		}
...
void idRenderProgManager::BindProgram( int index ) {
	if ( currentRenderProgram == index ) {
		return;
	}

	currentRenderProgram = index;
...
bool idRenderProgManager::ShaderHasOptionalSkinning() const
{
	return GetCurrentRenderProg().optionalSkinning;
}
...
void idRenderProgManager::PrintPipelineStates()
{
	for ( int i = 0; i < m_renderProgs.Num(); ++i ) 
	{
		renderProg_t & prog = m_renderProgs[ i ];
...
CONSOLE_COMMAND( Vulkan_PrintPipelineStates, "Print the GLState bits associated with each pipeline.", 0 ) {
	renderProgManager.PrintPipelineStates();
}