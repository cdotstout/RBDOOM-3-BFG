...
void idRenderBackend::AmbientPass( const drawSurf_t* const* drawSurfs, int numDrawSurfs, bool fillGbuffer )
{
	...
	*/
	...
	if( fillGbuffer )
	{
		...
		GL_CopyFrameBuffer(globalImages->currentNormalsImage, viewport.x1, viewport.y1, viewport.GetWidth(), viewport.GetHeight() );
...
void idRenderBackend::DrawScreenSpaceGlobalIllumination( const viewDef_t* _viewDef )
{
	...
	{
		const idScreenRect& viewport = viewDef->viewport;
		GL_CopyFrameBuffer(globalImages->currentRenderImage, viewport.x1, viewport.y1, viewport.GetWidth(), viewport.GetHeight() );
	}
...
void idRenderBackend::PostProcess( const void* data )
{
	...
	if( aaMode == ANTI_ALIASING_SMAA_1X )
	{
		...
		*/
		...
		GL_CopyFrameBuffer(globalImages->smaaInputImage, 0, 0, screenWidth, screenHeight );
